...
Reads all credentials from environment variables (GitHub Secrets).
Outputs: Email (Excel attached) + Google Drive upload + Google Sheets tab
"""

import os, json, pyotp, gspread, traceback, smtplib, base64
...
def get_creds():
    return {
        ...
        "sa_json":      env("SERVICE_ACCOUNT_JSON"),   # full JSON string
    }

def get_service_account_creds(sa_json_str, scopes):
    info = json.loads(sa_json_str)
    return Credentials.from_service_account_info(info, scopes=scopes)

# ── Styles ───────────────────────────────────────────────────
C = {
    "title_bg": "0D47A1", "hdr_mid":  "1976D2", "hdr_dark": "0D47A1",
...
INDEX_TOKENS = {
    ...
    "NIFTY IT":          ("NSE", "99926011"),
    "NIFTY SMALLCAP 50": ("NSE", "99926074"),
}

def fetch_indices(obj):
    result = {}
    for name, (exch, token) in INDEX_TOKENS.items():
        try:
            q     = obj.getQuote(exch, name, token, "SPOT")["data"]
...
NIFTY50 = [
    ...
    "KOTAKBANK","LT","SBIN","AXISBANK","BAJFINANCE","BHARTIARTL","M&M",
    "MARUTI","NESTLEIND","NTPC","ONGC","POWERGRID","SUNPHARMA","TATAMOTORS",
    "TATASTEEL","TECHM","TITAN","ULTRACEMCO","WIPRO","ADANIENT","ADANIPORTS",
    "APOLLOHOSP","ASIANPAINT","BAJAJFINSV","BAJAJ-AUTO","BEL","BPCL",
    "BRITANNIA","CIPLA","COALINDIA","DIVISLAB","DRREDDY","EICHERMOT",
    "GRASIM","HCLTECH","HEROMOTOCO","HINDALCO","INDUSINDBK","JSWSTEEL",
    "LTIM","SHRIRAMFIN","TATACONSUM","ZOMATO"
]
...
def fetch_nifty50(obj):
    ...
    for sym in NIFTY50:
        try:
            res   = obj.searchScrip("NSE", sym)
            token = next((i["symboltoken"] for i in (res.get("data") or [])
                          if i.get("tradingsymbol","").upper() == sym.upper()), None)
            if not token:
                stocks.append({"symbol": sym, "ltp": None, "chng": None, "pChng": None})
                continue
            q     = obj.getQuote("NSE", sym, token, "SPOT")["data"]
            ltp   = float(q.get("ltp",   0))
            close = float(q.get("close", ltp))
            chng  = round(ltp - close, 2)
            pct   = round((chng / close) * 100, 2) if close else 0.0
...
def build_excel(label, ist_dt, indices, stocks):
    ...
    for i, name in enumerate(["NIFTY 50","SENSEX","BANK NIFTY","NIFTY IT","NIFTY SMALLCAP 50"]):
        ...
        bg = C["idx_bg"] if i%2==0 else C["idx_alt"]; pos = pct is not None and pct >= 0
    ...
    for i, s in enumerate(stocks):
        sr += 1; pos=(s["pChng"] or 0)>=0
        bg=(C["pos_bg"] if pos else C["neg_bg"]) if i%2==0 else (C["pos_alt"] if pos else C["neg_alt"])
        ...
        ws.cell(sr,2,s["symbol"]).font=font(bold=True); ws.cell(sr,2).fill=fill(bg)
        ws.cell(sr,2).border=brd(); ws.cell(sr,2).alignment=aln()
    ...
    for col, w in {1:4,2:14,3:12,4:13,5:11,7:14,8:12,9:12,10:11,12:14,13:12,14:12,15:11}.items():
...
def send_email(creds, xlsx_path, label, ist_dt, indices, stocks):
    ...
    subject  = f"📊 NSE Snapshot {disp} IST — {date_str}"
    ...
    for name in ["NIFTY 50","SENSEX","BANK NIFTY","NIFTY IT","NIFTY SMALLCAP 50"]:
        d = indices.get(name,{}); pct = d.get("pct")
        if pct is not None:
            color = "#1b5e20" if pct>=0 else "#b71c1c"
            bg    = "#e8f5e9" if pct>=0 else "#ffebee"
            arrow = "▲" if pct>=0 else "▼"
            ltp   = d.get("ltp",0)
            idx_html += f'<tr style="background:{bg}"><td style="padding:7px 14px;font-weight:bold">{name}</td><td style="padding:7px 14px;color:{color};font-weight:bold;text-align:center">{arrow} {pct:+.2f}%</td><td style="padding:7px 14px;text-align:center">₹{ltp:,.2f}</td></tr>'
        else:
            idx_html += f'<tr><td style="padding:7px 14px;font-weight:bold">{name}</td><td style="padding:7px 14px;color:#9e9e9e;text-align:center">N/A</td><td></td></tr>'

    def stock_rows(lst, color, bg):
        return "".join(
            f'<tr style="background:{bg}"><td style="padding:6px 12px;font-weight:bold">{s["symbol"]}</td>'
            f'<td style="padding:6px 12px;color:{color};font-weight:bold;text-align:center">{s["pChng"]:+.2f}%</td>'
            f'<td style="padding:6px 12px;color:{color};text-align:center">₹{s["chng"]:+.2f}</td></tr>'
            for s in lst)

    html = f"""<html><body style="font-family:Arial,sans-serif;max-width:620px;margin:auto;padding:0">
...
<div style="border:1px solid #ddd;border-top:none;padding:18px;border-radius:0 0 10px 10px;background:#fff">
  <h3 style="color:#0d47a1;margin:0 0 10px">Index Performance</h3>
  <table width="100%" cellspacing="0" style="border-collapse:collapse;border:1px solid #e0e0e0;border-radius:6px;overflow:hidden">
    <tr style="background:#1976d2;color:white"><th style="padding:8px 14px;text-align:left">Index</th><th style="padding:8px">% Change</th><th style="padding:8px">LTP</th></tr>
    {idx_html}
  </table>
    ...
    <tr valign="top">
      <td width="50%" style="padding-right:8px">
        ...
        <table width="100%" cellspacing="0" style="border-collapse:collapse;border:1px solid #e0e0e0">
          {stock_rows(top3p,"#1b5e20","#e8f5e9")}
        </table>
      </td>
      <td width="50%" style="padding-left:8px">
        <h3 style="color:#c62828;margin:0 0 8px">📉 Top 3 Losers</h3>
        <table width="100%" cellspacing="0" style="border-collapse:collapse;border:1px solid #e0e0e0">
          {stock_rows(top3n,"#b71c1c","#ffebee")}
        </table>
      </td>
    </tr>
  </table>
  <div style="margin-top:16px;padding:14px;background:#f5f5f5;border-radius:8px;font-size:13px">
    📎 <b>Full Excel</b> with all 50 Nifty stocks is attached.<br>
    ☁️ Also saved to your <b>Google Drive → NSE Snapshots</b> folder.<br><br>
    💡 <b>To get data on demand:</b><br>
    &nbsp;&nbsp;→ Go to <a href="https://github.com">your GitHub repo</a> → Actions → NSE Market Snapshot → <b>Run workflow</b>
  </div>
</div></body></html>"""
...
def upload_drive(creds, xlsx_path):
    svc_creds = get_service_account_creds(
        creds["sa_json"], ["https://www.googleapis.com/auth/drive"])
    svc  = build("drive","v3",credentials=svc_creds)
...
def update_sheets(creds, label, ist_dt, indices, stocks):
    ...
    gc  = gspread.authorize(svc_creds)
    ...
    rows = [
        ...
        [""]*15,
        ["INDEX","LTP","CHANGE","% CHANGE",""]+["TOP 7 +ve","","","",""]+["TOP 7 -ve","","",""]
    ]
    for name in ["NIFTY 50","SENSEX","BANK NIFTY","NIFTY IT","NIFTY SMALLCAP 50"]:
    ...
    rows.append([""]*15)
    rows.append(["#","SYMBOL","LTP","CHG","% CHG","","SYMBOL","LTP","CHG","% CHG","","SYMBOL","LTP","CHG","% CHG"])
    valid=[s for s in stocks if s["pChng"] is not None]
    top7p=sorted(valid,key=lambda x:x["pChng"],reverse=True)[:7]
    top7n=sorted(valid,key=lambda x:x["pChng"])[:7]
    for i, s in enumerate(stocks):
...
def main():
    ...
    # Support manual label from workflow_dispatch
    ...
    try:
        ...
        print(f"\n✅ ALL DONE — {disp} IST snapshot complete!\n")
